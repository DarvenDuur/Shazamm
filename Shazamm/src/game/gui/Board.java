...
public class Board extends javax.swing.JPanel {
    ...
    private void initContents(int x, int y) {
        ...
        this.title = new ImageIcon(path + "logo_shazamm.gif").getImage();
        this.Bridge = new ArrayList<>();
        
        for(int i = 1; i < 20; i++){
        	if(i < 10){
            	Bridge.add(new ImageIcon(path + "\\pont\\pont_0" + i + ".gif").getImage());
        	}else{
            	Bridge.add(new ImageIcon(path + "\\pont\\pont_" + i + ".gif").getImage());
        	}
        }
    }
